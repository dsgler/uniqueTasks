...
let itemStr = ref("");
const { add_handler } = inject<liclass>("liclass")!;
...
<template>
  <div class="input-container myrow">
    <input v-model.trim="itemStr" @keydown.enter="add_handler(itemStr)" class="new-todo"
      placeholder="What needs to be done?" autofocus="true">
    <img :src="add_img" @click="add_handler(itemStr)" />
  </div>
</template>

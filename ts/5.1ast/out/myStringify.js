...
class myStringify {
    ...
    static resolveForStatement(d) {
        let js = `for (${this.autoRs(d.init)};${this.autoRs(d.test)};${this.autoRs(d.update)})`;
    ...
    static resolveConditionalExpression(d) {
        return `${this.autoRs(d.test)} ? ${this.autoRs(d.consequent)} : ${this.autoRs(d.alternate)}`;
    }
